...
function getData($timestamp = null)
{
    ...
    $sql_1 = is_null($timestamp)
        ? "SELECT * FROM points WHERE prediction IS NOT null ORDER BY timestamp ASC"
        : "SELECT * FROM points WHERE prediction IS NOT null AND timestamp > '$timestamp' ORDER BY timestamp ASC";
