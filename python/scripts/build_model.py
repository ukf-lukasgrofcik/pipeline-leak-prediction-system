...
def trainModel(features, epochs = 20, batch = 4, neurons = 512):
    ...
    model = Sequential([
        # Encoder
        Sequential([
            Dense(400, input_shape = (400,), activation = 'relu', kernel_regularizer = l1()),
            Dense(neurons_2, activation = 'relu', kernel_regularizer = l1()),
            Dense(neurons, activation = 'relu', kernel_regularizer = l1()),
        ]),
        # Decoder
        Sequential([
            Dense(neurons, input_shape = (neurons,), activation='relu', kernel_regularizer = l1()),
            Dense(neurons_2, activation = 'relu', kernel_regularizer = l1()),
...
for modelVersion in range(1, VERSIONS + 1):
    model = trainModel(features, EPOCHS, BATCH_SIZE, NEURONS)
